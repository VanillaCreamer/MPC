    ...
    def sample_pv(self):
        ...
        def sample_neg_items_for_u(u, num):
            ...
            while True:
                if len(neg_items) == num: break
                neg_id = np.random.randint(low=0, high=self.n_items,size=1)[0]
                if neg_id not in self.pv_set[u] and neg_id not in neg_items:
                    neg_items.append(neg_id)
            return neg_items

        def sample_neg_items_for_u_from_pools(u, num):
            neg_items = list(set(self.neg_pools[u]) - set(self.pv_set[u]))
            return rd.sample(neg_items, num)
    ...
    def sample_cart(self):
        ...
        def sample_neg_items_for_u(u, num):
            ...
            while True:
                if len(neg_items) == num: break
                neg_id = np.random.randint(low=0, high=self.n_items,size=1)[0]
                if neg_id not in self.cart_set[u] and neg_id not in neg_items:
                    neg_items.append(neg_id)
            return neg_items

        def sample_neg_items_for_u_from_pools(u, num):
            neg_items = list(set(self.neg_pools[u]) - set(self.cart_set[u]))
            return rd.sample(neg_items, num)
